...
/**
 * <p>
 * {@link TextureView} which can display animated GIFs. Available on API level
 * 14 ({@link Build.VERSION_CODES#ICE_CREAM_SANDWICH}) and above. GifTextureView
 * can only be used in a hardware accelerated window. When rendered in software,
 * GifTextureView will draw nothing.
 * </p>
 * <p>
 * GIF source can be specified in XML or by calling
 ...
 * </pre>
 * 
 * Note that <b>src</b> attribute comes from app namespace (you can call it
 * whatever you want) not from android one. Drawable, raw resources and assets
 * can be specified through XML. If value is a string (from resources or entered
 * directly) it will be treated as an asset.
 * <p>
 * Unlike {@link TextureView} GifTextureView is transparent by default, but it
 * can be changed by {@link #setOpaque(boolean)}. You can use scale types the
 * same way as in {@link android.widget.ImageView}.
 * </p>
 */
...
public class GifTextureView extends TextureView {
	...
	// public GifTextureView(Context context, AttributeSet attrs, int
	...
	private void init(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
		if (attrs != null) {
			...
			super.setOpaque(textureViewAttributes.getBoolean(
					R.styleable.GifTextureView_isOpaque, false));
	...
	/**
	 * Always throws {@link UnsupportedOperationException}. Changing
	 * {@link SurfaceTextureListener} is not supported.
	 * 
	 * @param listener
	 *            ignored
	 */
	...
	/**
	 * Always returns null since changing {@link SurfaceTextureListener} is not
	 * supported.
	 * 
	 * @return always null
	 */
	...
	/**
	 * Always throws {@link UnsupportedOperationException}. Changing
	 * {@link SurfaceTexture} is not supported.
	 * 
	...
	private class RenderThread extends Thread implements SurfaceTextureListener {
		...
		void dispose() {
			...
			final boolean isCallerInterrupted = Thread.currentThread()
					.isInterrupted();
	 ...
	 * content is assumed to be <b>non-opaque</b> by default (unlike
	 * {@link TextureView}. View that is known to be opaque can take a faster
	 * drawing case than non-opaque one.<br>
	 * Opacity change will cause animation to restart.
	 * 
	 * @param opaque
	 *            True if the content of this GifTextureView is opaque, false
	 *            otherwise
	 */
	@Override
	...
	/**
	 * Returns last {@link IOException} occurred during loading or playing GIF
	 * (in such case only {@link GifIOException} can be returned. Null is
	 * returned when source is not, surface was not yet created or no error
	 * occurred.
	 * 
	...
	public IOException getIOException() {
		if (mRenderThread.mIOException != null) {
			...
			return GifIOException.fromCode(mRenderThread.mGifInfoHandle
					.getNativeErrorCode());
		}
	}
	...
	/**
	 * Wrapper of {@link #setTransform(Matrix)}. Introduced to preserve the same
	 * API as in {@link GifImageView}.
	 * 
	 * @param matrix
	...
	/**
	 * Works like {@link TextureView#setTransform(Matrix)} but transform will
	 * take effect only if scale type is set to {@link ScaleType#MATRIX} through
	 * XML attribute or via {@link #setScaleType(ScaleType)}
	 * 
	 * @param transform
	 *            The transform to apply to the content of this view.
	 */
	...
	/**
	 * Returns the transform associated with this texture view, either set
	 * explicitly by {@link #setTransform(Matrix)} or computed according to the
	 * current scale type.
	 * 
	 * @param transform
	 *            The {@link Matrix} in which to copy the current transform. Can
	 *            be null.
	 * @return The specified matrix if not null or a new {@link Matrix} instance
	 *         otherwise.
	...
	public Parcelable onSaveInstanceState() {
		...
		return new GifViewSavedState(super.onSaveInstanceState(),
				mFreezesAnimation ? mRenderThread.mSavedState : null);
	}
	 ...
	 * {@link #onSaveInstanceState()} and restored in
	 * {@link #onRestoreInstanceState(Parcelable)}
	 * 
	 * @param freezesAnimation
	 *            whether animation position is saved
	 */
