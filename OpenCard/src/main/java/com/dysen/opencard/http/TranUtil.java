package com.dysen.opencard.http;
...
import com.dysen.commom_library.utils.HttpThread;
...
import com.dysen.socket_library.response.ResponseMsg2;
...
public class TranUtil {
    ...
    private static String transName;//本次交易名称
...
//    LogUtils.TAG = TranUtil.class.getName();
    ...
    public static RequestMsgHead2 packMsgHead2(String tellerNo, String tranCode){
        ...
        head2.AuthSeqNo.setValue("20180820001002003004");
        head2.BkSysDate.setValue("101010");
        head2.Bk10Date1.setValue("20180820");
        head2.BkDTime.setValue("20180820101010");
        head2.BkUuidNo.setValue("201808200001000200030004000511");
...
//              Integer.valueOf("8088").intValue(), timeout * 10000, rm.getMsg());
            ...
            String content="--------------------------------------本次交易开始--------------------------------"+"\n"+
                    ...
                    "交易流水号：="+preUUID+"\n"+
                    "--------------------------------------本次交易结束--------------------------------"+"\n\n";
            if(tranCode.equals("990011")){
                ToastUtil.LongToast(context,"990011交易进行");
            }
            transName = SocketThread.getTransStr(tranCode).substring(7, SocketThread.getTransStr(tranCode).length());
            FileUtils.writeTransLog( tranCode +"_"+transName +"_"+"requset", content+"生成请求报文"+"\n"+new String(requestByte, "gbk"));
            if(tranCode.equals("990011")){
                ToastUtil.LongToast(context,"990011交易进行写文件");
            }
            //            //日志发送给CRM后台，走Http请求
//            String parms=com.dysen.commom_library.utils.ParamUtils.setFkezsParams(ParamUtils.orgId,ParamUtils.tellerId,ParamUtils.tellerName,SocketThread.getTransStr(tranCode),accountType,productType);
//            HttpThread.sendRequestGet(ConstantValue.serverUrl,parms,handler,-1);
        ...
        try {
            ...
            FileUtils.writeTransLog( tranCode+"_"+transName +"_"+"response", "交易号:" + SocketThread.getTransStr(tranCode) +"\n"+"流水号：="+generateUUID()+"\n"+ "生成应答报文:"+"\n"+new String(retMsg, "gbk"));
     ...
     * @return
     */
    public static ResponseMsg2 startTran2(String tranCode, RequestMsgHead2 head, RequestMsgBody body, Context context, Handler handler) {
        RequestMsg rm = new RequestMsg(head, body,"");
        SocketManager sm = new SocketManager();
        d("hut", SocketThread.getTransStr(tranCode) + "---------交易提交开始");
        d("hut","请求IP地址：==" + ParamUtils.serverIp2);
        d("hut","请求IP端口：=="+         ParamUtils.serverPort2);
        d("hut", SocketThread.getTransStr(tranCode) +"---------生成发送报文---------------");
        d("hut","GDTA_ITEMDATA_LENGTH"+"---"+rm.GDTA_ITEMDATA_LENGTH.getLength()+"---"+rm.GDTA_ITEMDATA_LENGTH.getValue());
        d("hut","TRAN_CODE"+"---"+rm.TRAN_CODE.getLength()+"---"+rm.TRAN_CODE.getValue());
        d("hut","GDTA_SVCNAME"+"---"+rm.GDTA_SVCNAME.getLength()+"---"+rm.GDTA_SVCNAME.getValue());
...
//            HttpThread.sendRequestGet(ConstantValue.serverUrl,parms,handler,-1);
            String str=RequestMsg.requesgbkStr;
            retMsg = sm.transmitMsg(ParamUtils.serverIp2,
                    Integer.valueOf(ParamUtils.serverPort2).intValue(), timeout * 1000, requestByte,str);
        } catch (UnknownHostException e) {
            handler.sendEmptyMessage(-101);
            e.printStackTrace();
            LogUtils.e(e+"SocketManager 传输报文异常1"+e.getMessage());
        ...
        ResponseMsg2 result;
        try {
                result = new ResponseMsg2(retMsg, rrmb);
            LogUtils.e("应答报文不为空");
        } catch (Exception e) {
            e.printStackTrace();
            LogUtils.e("ResponseMsg 解析报文异常:" + e.getMessage());
       ...
       // dailog.show();
        ...
        try {
            ...
            if (transCode.equals(SocketThread.signIn)) {
                ...
                if (list != null && (list.size() == 13)||list.size()==15) {
                    ...
                    body.getMsgField("BkFlg").setValue(list.get(14));
                }
                ...
                if(list!=null && list.size() == 3){
                    body.getMsgField("MNG_tellerno").setValue(list.get(0));
                    body.getMsgField("FPI_tx_model").setValue(list.get(1));
                    body.getMsgField("BkDetail8").setValue(list.get(2));
                }
            }
        } catch (DocumentException e) {
            e.printStackTrace();
        ...
        if(transCode.equals(SocketThread.fingerCheck)||transCode.equals(SocketThread.tellerInfoSearch)){
            ...
            try {
                result=startTran2(transCode,head2,body,context,handler);
            } catch (Exception e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
            ...
            }else if(result.responseMsgHead.BkOthRetCode.getFieldValue().contains(GlobalData.RET_SUCCESS2)) {
                //日志发送给CRM后台，走Http请求
//            String parms=com.dysen.commom_library.utils.ParamUtils.setFkezsParams(ParamUtils.orgId,ParamUtils.tellerId,ParamUtils.tellerName,SocketThread.getTransStr(transCode),accountType,productType);
//            com.dysen.commom_library.utils.LogUtils.d("hut", "发送给CRM后台日志参数=sendRequest:"+parms);
//            HttpThread.sendRequestGet(ConstantValue.serverUrl,parms,handler,-1);
                // FileUtils.writeTransLog(transCode +"_"+ "onSuccess", "交易号:" + transCode + "交易执行成功"+"\n");
                responsList.clear();
                //返回交易头数据
                responsHeader.add(result.responseMsgHead.BILL_NO.getFieldValue());
                responsHeader.add(result.responseMsgHead.BkAreaNo.getFieldValue());
                responsHeader.add(result.responseMsgHead.BkPCode.getFieldValue());
                responsHeader.add(result.responseMsgHead.BkOthRetCode.getFieldValue());
                responsHeader.add(result.responseMsgHead.BkOthRetMsg.getFieldValue());
                responsHeader.add(result.responseMsgHead.BkTxCode.getFieldValue());
                responsHeader.add(result.responseMsgHead.BILL_TxDate.getFieldValue());
                responsHeader.add(result.responseMsgHead.TS_F_datetime.getFieldValue());
                responsHeader.add(result.responseMsgHead.BkOthOldSeq.getFieldValue());
                responsHeader.add(result.responseMsgHead.BkHostDate.getFieldValue());
                responsHeader.add(result.responseMsgHead.BkHostSeq.getFieldValue());
                responsHeader.add(result.responseMsgHead.BkPlatSeqNo.getFieldValue());
                responsHeader.add(result.responseMsgHead.TS_F_summry.getFieldValue());
...
//                }
                    ...
                    String s4=result.responseMsgBody.getMsgField("MNG_Tlrstat").getValue();
                    String s=result.responseMsgBody.getMsgField("MNG_Tlrlv").getValue();
                    String s5=result.responseMsgBody.getMsgField("MNG_grpno").getValue();
                    String s6=result.responseMsgBody.getMsgField("MNG_brchno1").getValue();
                    String s7=result.responseMsgBody.getMsgField("MNG_brchno2").getValue();
                    String s8=result.responseMsgBody.getMsgField("MNG_holidaystarttime").getValue();
                    String s19=result.responseMsgBody.getMsgField("MNG_holidayendtime").getValue();
                    String s10=result.responseMsgBody.getMsgField("MNG_startdate").getValue();
                    String s11=result.responseMsgBody.getMsgField("MNG_enddate").getValue();
                    String s12=result.responseMsgBody.getMsgField("MNG_idtype").getValue();
                    LogUtils.i("---test--*"+s1+"*-*"+s2+"*-*"+s3+"*-*"+s4+"*-*"+s+"*-*"+s5+"*-*"+s6+"*-*"+s7+"*-*"+s8);
                    LogUtils.i("---MNG_Tlrlv---"+s);
                    responsList.add(s);
                }
                ...
                String parms=com.dysen.commom_library.utils.ParamUtils.setFkezsParams(ParamUtils.orgId,ParamUtils.tellerId,ParamUtils.tellerName,SocketThread.getTransStr(transCode),accountType,productType);
                LogUtils.d("hut", "发送给CRM后台日志参数=sendRequest:"+parms);
                HttpThread.sendRequestGet(ParamUtils.CrmUrl,parms,handler,-1);
                ...
                Message message = new Message();
//                message.obj = respMsg;
                message.obj = result.responseMsgHead.BkOthRetCode.getValue();
                message.what=-3;
                handler.sendMessage(message);
//            FileUtils.writeTransLog( transCode  +"_"+"onFaile", "交易号:" + transCode + "交易执行失败"+"\n"+"流水号：="+generateUUID()+"\n"+result.ERR_MSG.getValue());
                LogUtils.e(transCode + "=错误信息="+result.responseMsgHead.BkOthRetCode.getValue());
